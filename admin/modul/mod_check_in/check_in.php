...
$aksi="modul/mod_check_in/aksi_check_in.php";
switch($_GET[act]){
  // Tampil Barcode
  default:
    echo "<h2>Daftar Checkin</h2>
    ...
    if ($_SESSION[leveluser]=='admin'){
      $tampil=mysql_query("SELECT barcode.id, barcode.kd_barcode, barcode.id_pemegang, barcode.`keterangan`, pemegang.nama_pemegang, pemegang.no_telp FROM barcode INNER JOIN pemegang ON barcode.id_pemegang = pemegang.id where check_in='Y' LIMIT $posisi,$batas");
    }
    ...
    while ($r=mysql_fetch_array($tampil)){
      echo "<tr><td class='left' width='25'>$no</td>
                ...
                <td class='center'  width='85'>
	                  <a href=$aksi?module=check_in&act=batal&id=$r[id]><img src='images/batal_check_in.png' border='0' title='hapus' /></a></td>
		        </tr>";
      $no++;
    }
    echo "</tbody></table>";
    ...
    if ($_SESSION[leveluser]=='admin'){
      $jmldata = mysql_num_rows(mysql_query("SELECT * FROM barcode where check_in='Y'"));
    }
...
echo "<div class=\"pagination\"> $linkHalaman</div>";
  ...
  case "tambahbarcode":
    echo "<h2>Tambah Barcode</h2>
          <form method=POST action='$aksi?module=check_in&act=input'>
  ...
  case "editbarcode":
    ...
    echo "<h2>Edit Barcode</h2>
          <form method=POST action=$aksi?module=check_in&act=update>
